...
_SUBMIT_TEXTS = ("Place Bet", "Place bet", "Confirm", "Submit")
_STAKE_SELECTORS = (
    'input[type="number"]',
...
async def _click_selection(page, selection: str) -> bool:
    ...
    for token in tokens[:3]:
        try:
            ...
            continue
    try:
        await page.get_by_role("button", name=re.compile(re.escape(selection[:12]), re.I)).first.click(timeout=5000)
        return True
    except Exception:
        return False
...
async def place_stake_bet(page, order: BetOrder, screenshot_dir: Path) -> dict:
    screenshot_dir.mkdir(parents=True, exist_ok=True)

    if not await _click_selection(page, order.selection):
        await page.screenshot(path=str(screenshot_dir / "stake_select_fail.png"))
        return {"status": "error", "step": "select", "message": f"Could not find: {order.selection}"}

    await page.wait_for_timeout(800)
    if not await _fill_stake(page, order.stake):
        await page.screenshot(path=str(screenshot_dir / "stake_stake_fail.png"))
        return {"status": "error", "step": "stake", "message": "Could not fill stake input"}

    await page.screenshot(path=str(screenshot_dir / "stake_presubmit.png"))
    if not await _submit(page):
        return {"status": "error", "step": "submit", "message": "Could not click Place Bet"}

    await page.wait_for_timeout(2000)
    await page.screenshot(path=str(screenshot_dir / "stake_postsubmit.png"))
    return {"status": "placed", "platform": "stake", "order": order}